...
def apply_user_data_filters(user, queryset, model_name):
    ...
    try:
        module = Module.objects.get(name='properties')
        ...
        for data_filter in filters:
            if data_filter.filter_conditions:
                try:
                    ...
                    queryset = queryset.filter(**data_filter.filter_conditions)
                except Exception as e:
                    # Log the error but don't break the view
                    print(f"Error applying filter {data_filter.name}: {e}")
                    continue
...
@login_requireduts import render, get_object_or_404, redirect
from django.contrib.auth.decorators import login_required
from django.contrib import messages
from django.db.models import Q, Count
from django.http import JsonResponse, HttpResponse
